...
const Db2QueryInterface = require('./dialects/db2/query-interface');
...
class QueryInterface {
  ...
  changeColumn(tableName, attributeName, dataTypeOrOptions, options) {
    ...
    if (this.sequelize.options.dialect === 'sqlite') {
      ...
      const query = this.QueryGenerator.attributesToSQL(attributes, {context: 'changeColumn'});
  ...
  getForeignKeyReferencesForTable(tableName, options) {
    ...
    let catalogName = this.sequelize.config.database;
    switch (this.sequelize.options.dialect) {
      case 'sqlite':
      ...
      case 'db2':
        catalogName = this.sequelize.config.username.toUpperCase();
      case 'mysql':
      default:
      {
