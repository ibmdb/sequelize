...
class Model {
  ...
  static bulkCreate(records, options = {}) {
    ...
    if (options.ignoreDuplicates && ['postgres', 'mssql', 'db2'].includes(dialect)) {
  ...
  save(options) {
    ...
    // Db2 do not allow NULL values for unique columns.
    // Add dummy values if not provided by test case or user.
    if (this.sequelize.options.dialect === 'db2' && this.isNewRecord) {
      for (var key in this.constructor.rawAttributes) {
        if (this.constructor.rawAttributes[key].unique &&
            this.dataValues[key] === undefined) {
          console.log(key, this.constructor.rawAttributes[key].type);
          if (this.constructor.rawAttributes[key].type instanceof DataTypes.DATE) {
            this.dataValues[key] = Utils.now(this.sequelize.options.dialect);
          } else if (this.constructor.rawAttributes[key].type instanceof DataTypes.STRING) {
            this.dataValues[key] = "unique" + this.uniqno++;
          }else if (this.constructor.rawAttributes[key].type instanceof DataTypes.INTEGER) {
            this.dataValues[key] = this.uniqno++;
          }else if (this.constructor.rawAttributes[key].type instanceof DataTypes.BOOLEAN) {
            this.dataValues[key] = new DataTypes.BOOLEAN(false);
          }
        }
      }
    }
