  ...
  _run(connection, sql, parameters) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (_.startsWith(this.sql, 'BEGIN TRANSACTION')) {
        ...
        connection.query(this.sql, params, (err, results) => {
          ...
          if (err && err.message && err.message.search("SQL0204N") != -1) {
            err = null; // Ignore table not found error in case of drop table.
          }
          if (err) {
            err.sql = sql;
            reject(this.formatError(err));
