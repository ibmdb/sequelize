...
class Db2QueryGenerator extends AbstractQueryGenerator {
  ...
  dropSchema(schema) {
    ...
    return [
      'IF EXISTS (SELECT schemaname',
      'FROM syscat.schemata',
      'WHERE schemaname =', quotedSchema, ')',
      'BEGIN',
      'DECLARE @id INT, @sql VARCHAR(2000);',
      'DECLARE @cascade TABLE (',
      'id INT NOT NULL GENERATED ALWAYS AS IDENTITY PRIMARY KEY,',
      'sql VARCHAR(2000) NOT NULL );',
      'INSERT INTO @cascade ( sql )',
      "SELECT CASE WHEN o.type IN ('F','PK')",
      "THEN 'ALTER TABLE '+ s.name + '.' + p.name + ' DROP CONSTRAINT ' + o.name ",
      "ELSE 'DROP TABLE '+ s.name + '.' + o.name END",
      'FROM sys.objects o',
  ...
  showSchemasQuery() {
    return 'SELECT "name" as "schemaname" FROM syscat.schemata';
  }
  ...
  attributeToSQL(attribute, options) {
    ...
    if (attribute.allowNull === false) {
      template += ' NOT NULL';
    } else if (attribute.allowNull === true && !attribute.primaryKey && !Utils.defaultValueSchemable(attribute.defaultValue)) {
      template += ' NULL';
    }
    ...
    if (attribute.autoIncrement) {
      ...
      template += ' GENERATED ALWAYS AS IDENTITY(START WITH ' +
  ...
  addLimitAndOffset(options, model) {
    ...
    if (options.limit || options.offset) {
      if (!options.order || options.include && !orders.subQueryOrder.length) {
        ...
        } else if(options.tableAs) {
            fragment += this.quoteTable(options.tableAs);
        }
      }

      // OFFSET is not supported by Db2 for LUW.
