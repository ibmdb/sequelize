...
class Db2QueryGenerator extends AbstractQueryGenerator {
  ...
  describeTableQuery(tableName, schema) {
    let sql = [
      ...
      'COLTYPE AS "Type", LENGTH AS "Length", SCALE AS "Scale",',
  ...
  showIndexesQuery(tableName) {
    const sql = 'SELECT NAME AS "name", TBNAME AS "tableName", UNIQUERULE AS "keyType", COLNAMES, INDEXTYPE AS "type" FROM SYSIBM.SYSINDEXES WHERE TBNAME = <%= tableName %>;';
