...
class ConnectionManager extends AbstractConnectionManager {
  constructor(dialect, sequelize) {
    ...
    if (sequelize.config.pool) {
      let poolOption = {};
      let poolObj = sequelize.config.pool;
      let min = poolObj.min || poolObj.init || 0;
      if (poolObj.max) {
        poolOption["maxPoolSize"] = poolObj.max;
      }
      if (poolObj.idle) {
        poolOption["idleTimeout"] = poolObj.idle;
        poolOption["autoCleanIdle"] = true;
      }
      if (sequelize.config.dialectOptions) {
        if (sequelize.config.dialectOptions.connectTimeout) {
          poolOption["connectTimeout"] =
              sequelize.config.dialectOptions.connectTimeout;
        }
        if (sequelize.config.dialectOptions.systemNaming) {
          poolOption["systemNaming"] =
              sequelize.config.dialectOptions.systemNaming;
        }
        if (sequelize.config.dialectOptions.poolInit && !min) {
          min = sequelize.config.dialectOptions.poolInit;
        }
        if (sequelize.config.dialectOptions.debug) {
          this.lib.debug(true);
        }
      }

      this.connPool = new this.lib.Pool(poolOption);

      if (min) {
        if (min > poolObj.max) {
          min = poolObj.max;
        }
        sequelize.config.pool["min"] = min;
        this.connPool.init(min, this.getdb2Config(sequelize.config));
      }
    }
